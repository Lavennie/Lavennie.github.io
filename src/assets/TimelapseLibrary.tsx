...
export function TimelapseById({ id, style } : TimelapseByIdProps) {
    const data = TimelapseLibrary.find((item) => item.id === id);
    ...
    return (
        <div className={"timelapse-frame"} style={style}>
            <Timelapse videoUrl={data.videoUrl} thumbnailUrl={data.thumbnailUrl} />
        </div>);
}
export function timelapseListHeight() {
    let cumulativeY= 0;
    for (const item of TimelapseLibrary) {
        cumulativeY += item.yGapAfter;
    }
    return cumulativeY;
}
export function CreateTimelapseList({startTop = 0}) {
    let cumulativeY = Number(startTop) || 0;

    return TimelapseLibrary.map((item) => {
        let element;
        if (item.videoUrl === "YEAR"){
            element = (
              <div style={{
                  position: "absolute",
                  top: `${cumulativeY}px`,
